...
import {
  ...
  MoreHorizontal, Play, Pause, SkipBack, SkipForward,
} from "lucide-react";
...
export function LyricsPanel({ open, onClose, trackId, trackTitle, trackArtist, coverUrl }: LyricsPanelProps) {
  ...
  const currentTime = previewProgress; // previewProgress is already currentTime in seconds
  ...
  // Scroll active line to center using getBoundingClientRect
  useEffect(() => {
    ...
    // line position inside scrollable content
  ...
  useEffect(() => {
    ...
    get<{ lyrics: string; synced_lyrics: string }>(`/api/tracks/${trackId}/lyrics`)
      .then(d => {
        setLyrics(d.lyrics ?? "");
        setLrcLines(d.synced_lyrics ? parseLrc(d.synced_lyrics) : []);
      })
  ...
  useEffect(() => {
    setSearchQuery(trackArtist ? `${trackArtist} ${trackTitle}` : trackTitle);
  ...
  async function handlePickItunes(result: ItunesResult) {
    ...
    try {
      ...
      setLrcLines(found.synced_lyrics ? parseLrc(found.synced_lyrics) : []);
      await post(`/api/tracks/${trackId}/lyrics`, { lyrics: found.lyrics ?? "" });
      if (found.synced_lyrics) {
        await post(`/api/tracks/${trackId}/lyrics/synced`, { synced_lyrics: found.synced_lyrics });
      }
      setMode("view");
    } catch { setSearchError(true); }
    finally { setFetching(false); }
  ...
  async function handleEditSave() {
    setSaving(true);
    try {
      await post(`/api/tracks/${trackId}/lyrics`, { lyrics: editText });
      setLyrics(editText); setLrcLines([]); setMode("view");
                  ...
                  {menuOpen && (
                    ...
                    >
                      ...
                      <button onClick={() => { setEditText(lyrics); setMode("edit"); }}
                        className="flex items-center gap-2.5 w-full px-4 py-3 text-sm text-white hover:bg-white/10 transition-colors">
                        <Edit2 className="size-4 text-white/60" /> 직접 편집
                      </button>
                    </motion.div>
                  )}
                </AnimatePresence>
              ...
              <div className="px-5 pb-4">
                <form onSubmit={handleSearch} className="flex gap-2 mb-3">
                  <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                    autoFocus
                    placeholder="아티스트 제목으로 iTunes 검색"
                ...
                {searchError && <p className="text-center text-red-400/80 text-sm mb-2">가사를 찾을 수 없습니다</p>}
                {showResults && (
            ...
            {mode === "edit" && (
              <div className="px-5 pb-4 flex-1 flex flex-col gap-3">
                <textarea value={editText} onChange={e => setEditText(e.target.value)}
                  autoFocus
                  className="flex-1 min-h-[200px] bg-white/5 text-white/90 rounded-xl p-4 text-sm font-mono leading-relaxed resize-none outline-none border border-white/10 focus:border-white/20 transition-colors"
            ...
            {mode === "view" && (
              ...
              >
                {loading ? (
                  ...
                  <pre className="text-white/80 text-lg leading-relaxed whitespace-pre-wrap pt-4"
                  ...
                  <div className="flex flex-col items-center pt-24 gap-3 text-white/30">
                    ...
                    <p className="text-sm">우측 상단 ··· 메뉴에서 검색하세요</p>
                  </div>
                )}
              </div>
            )}
